...
def findMedianSortedArrays( nums1: list[int], nums2: list[int]) -> float:
    ...
    # half is the size of the left part if the two list merged together
    ...
    while True:
        i = (r+l)//2     # from nums1 we select nums1[0:i+1]
        j = half - i - 2      # from nums2 we select nums2[0:j+1]
        nums1_left = nums1[i] if i>= 0 else float('-infinity')  # the biggest value in nums1[0:i+1]
        nums1_right = nums1[i+1] if i < len(nums1)-1 else float('infinity') # the smallest value in nums1[i+1::]
        nums2_left = nums2[j] if j>=0 else float('-infinity')  # the biggest value in nums2[0:j+1]
        nums2_right = nums2[j+1] if j < len(nums2)-1 else float('infinity') # the smallest value in nums2[j+1::]
        # end loop
        if nums1_left <= nums2_right and nums1_right >= nums2_left:
            ...
            if total%2 ==0:
                ...
                return right_min
        # nums1_left > nums2_right , decrease i
        elif nums1_left > nums2_right:
            r = i -1
        # nums1_right < nums2_left , increase i
        else:
            l = i + 1

# solution3 binary sort  
# idea comes from the first comment from  https://leetcode.cn/problems/median-of-two-sorted-arrays/comments/ 
...
def find_k_large(A,B,k):
    ''' find k'th largest number from A,B,Aand B is sorted   O(log(m+n))
    '''
    if A == []:
        return B[k-1]
    elif B == []:
